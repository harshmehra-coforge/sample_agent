...
async def create_user_stories(prompt: str , conversation_id: str , namespace:dict , file: list|None = None ,job_id:str | None = None , agent_feed:list | None = None )-> dict[str, Any]:
    ...
    # ---------------------- Vague input disambiguation ---------------------------
    disambiguation_result = llm.invoke(
        sys_prompt=cus_agent["vague_input_disambiguator"],
        input=requirements,
        history=[]
    )

    if "VAGUE_TERMS_FOUND" in disambiguation_result:
        vague_lines = disambiguation_result.replace("VAGUE_TERMS_FOUND", "").strip()
        vague_warning = (
            "**Vague Language Detected**\n\n"
            "The following terms in your requirements are unmeasurable. "
            "Please provide specific, quantifiable definitions for each before a high-quality BRD can be produced:\n\n"
            + vague_lines
            + "\n\nOnce you have clarified these terms, please re-submit your requirements."
        )
        logger.info("Vague input detected — returning disambiguation prompt to user.")
        session.append_message(conversation_id, "assistant", vague_warning, job_id=job_id)
        return {"status": "success", "role": "assistant", "content": vague_warning}
    ...
    ***High-level requirement details***:\n{requirements}\n\n"""
    ...
    # ---------------------- Section completeness scoring (internal) ---------------------------
    scorecard = llm.invoke(
        sys_prompt=cus_agent["section_completeness_scorer"],
        input=response,
        history=[]
    )
    logger.debug(f"Section completeness scorecard:\n{scorecard}")

    # ---------------------- Gap detection (shown to user above BRD) ---------------------------
    gap_report = llm.invoke(
        sys_prompt=cus_agent["gap_detector"],
        input=scorecard,
        history=[]
    )
